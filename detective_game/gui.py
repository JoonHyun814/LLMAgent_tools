...
# 7. Gradio
# -------------------------------------
def select_character(selected, turn, person_player, log_history):
    person_player = selected
    description = "\n".join(player_dict[person_player])
    log_history += f"'{selected}' 캐릭터로 게임을 시작합니다.\n<{person_player}의 비밀 정보>\n{description}\n"
    
    current_player = player_list[turn % len(player_list)]
    log_history += f"\n[{current_player}의 턴 시작]\n"
    if current_player == person_player:
        log_history + "\n명령을 입력하고 [다음 턴]을 누르세요."
    return log_history, gr.update(visible=(current_player == person_player)) , turn, person_player, log_history, gr.update(visible=False), gr.update(visible=False)

def advance_turn(user_input, turn, current_player, person_player, log_history):
    current_player = player_list[turn % len(player_list)]
    next_player = player_list[(turn+1) % len(player_list)]
    
    if current_player == person_player:
        log_history += f"{current_player}의 명령: {user_input}\n"
        result = f"(에이전트 응답 예시)"
        turn += 1
    else:
        # LLM으로부터 명령 생성
        llm_command = "(LLM 응답 예시)"
        log_history += f"{current_player}의 명령: {llm_command}\n"
        # 여기에 agent_executor.invoke 로직
        result = f"(에이전트 응답 예시)"
        turn += 1
    ...
    log_history += f"결과: {result}\n"
    log_history += f"\n[{next_player}의 턴 시작]\n"
    return log_history, gr.update(visible=(next_player == person_player),value=""), turn, current_player, person_player, log_history

with gr.Blocks() as demo:
    gr.Markdown("## Crime Scene")
    gr.Textbox(game_story_prompt, label ="게임 스토리")
    ...
    with gr.Row():    
        char_selector = gr.Dropdown(choices=player_list, label="당신의 캐릭터 선택")
    ...
    select_button.click(
        select_character, 
        inputs=[char_selector, turn, person_player, log_history], 
        outputs=[output_box, user_input, turn, person_player, log_history, select_button,char_selector]
    )
    next_button.click(
        advance_turn, 
        inputs=[user_input, turn, current_player, person_player, log_history], 
        outputs=[output_box, user_input, turn, current_player, person_player, log_history]
    )
